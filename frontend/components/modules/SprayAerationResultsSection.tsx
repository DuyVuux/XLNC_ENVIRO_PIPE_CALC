      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6">
        {outputs.C_sat && (
          <div>
            <label className="block text-base font-medium text-gray-600 dark:text-gray-400">
              Saturated oxygen concentration C_sat
            </label>
        ...
        {outputs.O2_required && (
          <div>
            ...
            <p className="mt-1 text-xl font-semibold text-gray-900 dark:text-gray-100 bg-gray-100 dark:bg-gray-700 p-3 rounded-md min-h-[44px]">
              {formatValue(outputs.O2_required)}
            </p>
          </div>
        )}
        {outputs.efficiency && (
          <div>
            <label className="block text-base font-medium text-gray-600 dark:text-gray-400">
              Aeration efficiency η
            </label>
            <p className="mt-1 text-xl font-semibold text-gray-900 dark:text-gray-100 bg-gray-100 dark:bg-gray-700 p-3 rounded-md min-h-[44px]">
              {formatValue(outputs.efficiency, "%")}
            </p>
          </div>
        )}
        {outputs.removal_rate && (
          <div>
            <label className="block text-base font-medium text-gray-600 dark:text-gray-400">
              Removal rate
            </label>
            <p className="mt-1 text-xl font-semibold text-gray-900 dark:text-gray-100 bg-gray-100 dark:bg-gray-700 p-3 rounded-md min-h-[44px]">
              {formatValue(outputs.removal_rate)}
            </p>
          </div>
        )}
      </div>
